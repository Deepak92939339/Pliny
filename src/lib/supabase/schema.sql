...
create table if not exists public.documents (
  ...
  processing_stage text not null default 'uploading' check (processing_stage in ('validating', 'uploading', 'extracting', 'ocr_fallback', 'chunking', 'embedding', 'indexing', 'ready', 'failed')),
...
-- previously created for another provider, reset derived embeddings and
-- backfill them with Voyage after running this schema.
do $$
begin
  if exists (
    select 1
...
as $$
  ...
  select
    dc.id, dc.document_id, dc.collection_id, dc.content, dc.file_kind,
    dc.location_label, dc.metadata, dc.page_number, dc.chunk_index,
    (ts_rank_cd(dc.lexical_search, query.terms, 32) +
      case when to_tsvector('simple', d.filename) @@ query.terms then 0.25 else 0 end)::real as lexical_rank
  from public.document_chunks dc
  join public.documents d on d.id = dc.document_id and d.collection_id = dc.collection_id
  cross join query
  where dc.collection_id = match_collection_id
...
-- Phase 4B privacy-minimised processing (reviewed migration proposal mirror).
