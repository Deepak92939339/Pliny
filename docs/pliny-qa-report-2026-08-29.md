...
## PHASE 4B.2A AUTHORITATIVE RELEASE RESULT — 2026-09-02

This section is authoritative for the privacy-minimised processing foundation and anonymous Data API hardening release. Earlier report sections remain historical evidence for other QA phases and do not override the classifications below.

### Release identity and migration history

- Accepted privacy checkpoint: `8c77d097336e53400fe002e689b5c79885bfb6a8`.
- Anonymous privilege-hardening commit: `cac6681ce41a92fd5215df8d26656f7d99e15869`.
- Both commits were pushed by fast-forward to GitHub `main`; no earlier commit was amended or rewritten.
- Phase 4B migration: `20260901044022_phase_4b_privacy_minimised_processing.sql`, accepted SHA-256 `5147cf4cad0a2abb7cb22f574a5c0220333bdddede92d8ce9fa69e9586bb2ab7`.
- Phase 4B.2A migration: `20260902043330_revoke_anon_private_application_table_dml.sql`, SHA-256 `0430e163c3d950db131dd2b6b9121a3340ec33c953ecbc0f7ea8942ddea64b07`.
- Linked Production migration history is aligned exactly through `20260830183604`, `20260830214649`, `20260831090000`, `20260901044022` and `20260902043330`. The Phase 4B.2A dry run contained only `20260902043330`; it applied once without warning or history repair.
- Matching application deployment: `dpl_6cshtnfVp1EBmPxAuoWyamc3d6CX`, Production, READY, commit `cac6681ce41a92fd5215df8d26656f7d99e15869`, aliased to `https://pliny.vercel.app`.

### Database and authorization witnesses

- The Phase 4B.2A migration revokes only `SELECT`, `INSERT`, `UPDATE` and `DELETE` from `anon` on `public.collections`, `public.documents`, `public.document_chunks` and `public.chat_messages`. It changes no authenticated or service-role grant, RLS policy, function, sequence, Storage policy, schema or default privilege.
- Catalog witnesses proved the former grants were direct to `anon`, not inherited from `PUBLIC` or another role. After application, both direct ACLs and `has_table_privilege` report all four DML privileges false for `anon` on all four tables; `authenticated` and `service_role` retain their preceding DML matrix.
- Anonymous table access now fails at the ACL layer with SQLSTATE `42501`. Authenticated owner reads, legitimate writes, mode capture, unrelated updates and standard/privacy retrieval pass. Authenticated non-owner reads, mutations, citations and RPC access return no owner data.
- All four tables retain RLS. Collection and document UPDATE policies retain both `USING` and `WITH CHECK`. The Phase 4B lexical RPC privilege matrix is unchanged. Both Phase 4B functions remain `SECURITY INVOKER` with their accepted search paths; PUBLIC, anon and service-role execution remain denied and authenticated receives only the accepted RPC execution grant.
- `document_chunks.embedding` remains `vector(1024)`. The provider-safe generated lexical column and valid masked GIN index remain present. Existing documents remain three standard documents; no masked data was fabricated.

### Counts, local acceptance and advisors

- Production before/after counts are identical: 3 collections, 3 documents, 11 chunks, 46 chat messages, 19 citations, 5 Storage objects and 43,841,029 Storage bytes. The `documents` bucket remains private.
- A disposable PostgreSQL 17 acceptance target rebuilt the complete chain twice with byte-identical public schema dumps, SHA-256 `27fb389250d899a145be3752ebeb071f6badb7ed5726e53a85d09ca0d66c60e0`. The expanded pgTAP suite passed 59/59 assertions on both builds, migration-up reported no pending migration afterward, and database lint passed. The verified disposable instance was then stopped without backup.
- Security Advisor retains the established public `vector` extension warning and an existing leaked-password-protection warning. Performance Advisor reports expected low-volume unused-index notices. This bounded release did not change unrelated privileges, Auth configuration or index design.

### Production compatibility and secrets

- Exactly one `PRIVACY_PSEUDONYM_KEY` was witnessed in Vercel Production scope with sensitive storage. Its value was never requested or exposed. The variable is server-only and has no `NEXT_PUBLIC_` prefix.
- Production root, login and signup returned 200. Protected dashboard and collection routes redirected unauthenticated users to login. Chat, search, process-document and upload APIs returned 401 to deliberate body-free unauthenticated probes before provider or upload handling.
- Sixteen referenced Production browser JavaScript assets (762,183 bytes) contained zero occurrences of `PRIVACY_PSEUDONYM_KEY` or `NEXT_PUBLIC_PRIVACY_PSEUDONYM_KEY`; the full local Production bundle scan also passed.
- Deployment-scoped logs showed only the four expected missing-session records generated by the deliberate 401 probes. There was no 5xx witness or unexpected runtime fault, and a scoped search found no privacy-key identifier.
- Privacy, privacy-bundle, ingestion, retrieval, embeddings, evidence, citations, sanitization and report tests passed. Offline evaluation passed 14/14; live evaluation was not run. Lint, TypeScript, Production build, diff check and `npm audit --omit=dev` passed; the production dependency audit reported zero vulnerabilities.

### Phase verdict and remaining gates

- Local database acceptance: **PASS**.
- Production schema release: **PASS**.
- Anonymous Data API privilege hardening: **PASS**.
- Provider-free Production compatibility: **PASS**.
- Voyage opt-out: **UNVERIFIED**.
- Live privacy-mode processing: **NOT RUN**.
- GLM integration: **NOT STARTED**.
- Provider requests in Phase 4B.2/4B.2A: **zero**.
- Document uploads/transmissions in Phase 4B.2/4B.2A: **zero**.
