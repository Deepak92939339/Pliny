...
The first pull-request quality-gate run also exposed an honest CI-only defect: the workflow selected Node 20 while the committed deterministic scripts require Node's `--experimental-strip-types` support. The repair moves the pinned setup to Node 22 and adds a provider-free configuration regression that rejects unsupported Node majors and provider-secret references. The exact-commit GitHub rerun passed lockfile installation, lint, typecheck, all deterministic regressions and the frozen evaluation without provider credentials. No application runtime behavior changed.

## Private-beta release blocker closure — 2026-09-10

The remaining public-signup failure was resolved by narrowing the product honestly rather than claiming an untested email flow. Pliny is now explicitly an invitation-only private beta. Landing and information calls to action lead to login, `/signup` is a static access-by-invitation explanation with no form, and the login surface no longer imports or exposes signup behavior. The retained server action rejects every signup input before creating a Supabase client. The isolated Preview Supabase Auth configuration also rejects direct anonymous signup, closing the provider endpoint around the application.

A focused real-browser Preview smoke used one administrator-created, confirmed synthetic user. It passed nine assertions covering the private-beta landing and access pages, absence of signup forms/links, unauthenticated protected-route redirect, invalid login, confirmed-user login, session persistence after reload, logout/session clearing, zero browser console issues and zero Preview 5xx responses. The synthetic user was removed. This phase made zero OpenRouter and zero Voyage requests at `$0.00` cost because answer and retrieval behavior were unchanged.

The migration review was also widened beyond the earlier public-schema comparison. Storage bucket configuration, Storage RLS policies and grants, schema privileges, and effective routine privileges are now fingerprinted. That check found one real Preview defect: the foundational function creation left execute available through `PUBLIC`, so revoking only `anon` did not secure a fresh environment. Production already had the intended restrictive grant. A forward-only migration now revokes `PUBLIC` and `anon` execute on `match_document_chunks` and grants only `authenticated`; it was applied to Preview, after which all 14 realized-state sections matched Production exactly.

This evidence makes migration-history repair safe under an explicit precondition: rerun the SELECT-only comparison immediately before the change and require no realized-state differences. Then mark only `20260830000000` applied without executing its SQL, dry-run the ordinary migration workflow, require the new forward grant repair to be the sole pending migration, and apply it. The complete command order, Production-only runtime-variable preflight, validation requirements and rollback boundaries are documented in `docs/private-beta-production-deployment-runbook.md`.

The resulting recommendation is **GO for a controlled Production deployment procedure within the private-beta scope**, subject to explicit Production authorization and a green exact-head PR gate. It is not a claim that public signup works, and public self-service access must remain disabled until a separately tested product phase restores it.
