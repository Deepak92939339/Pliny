...
This evidence makes migration-history repair safe under an explicit precondition: rerun the SELECT-only comparison immediately before the change and require no realized-state differences. Then mark only `20260830000000` applied without executing its SQL, dry-run the ordinary migration workflow, require the new forward grant repair to be the sole pending migration, and apply it. The complete command order, validation requirements and rollback boundaries are documented in `docs/private-beta-production-deployment-runbook.md`.
